...
pacman-key --init
pacman-key --populate archlinux
pacman -Sy xorg xorg-server mate mate-extra lxdm sudo git
...
stemctl start lxdm.service

sleep 1
echo "password for root user:"
passwd
echo "add new user"
useradd -mg users -G wheel -s /bin/bash kolmogorov
echo "paaswd for new user"
passwd

sleep 1
echo "Hostname"
hostnamectl set-hostname evpz 
